...
var multiple: bool:
	get:
		return object_connection_pairs.size() > 1
...
func update_object_connection(pair: ObjectConnectionPair) -> void:
	...
	for old_signal_connection_candidate in pair.from_object.get_signal_connection_list(pair.signal_name):
		if !old_signal_connection_candidate.callable.is_valid():
			continue
		if (old_signal_connection_candidate.flags & CONNECT_PERSIST) == 0:
			continue
	...
	if _deferred != null:
		if _deferred:
			new_flags |= CONNECT_DEFERRED
		else:
			new_flags &= ~CONNECT_DEFERRED
	if _one_shot != null:
		if _one_shot:
			new_flags |= CONNECT_ONE_SHOT
		else:
			new_flags &= ~CONNECT_ONE_SHOT
	if _append_source != null:
		if _append_source:
			new_flags |= CONNECT_APPEND_SOURCE_OBJECT
		else:
			new_flags &= ~CONNECT_APPEND_SOURCE_OBJECT
...
func _get(property: StringName) -> Variant:
	match property:
		...
		&"unbind_signal_arguments":
			return _unbind_signal_arguments
		&"bound_signal_arguments":
			return _bound_signal_arguments
	return null
