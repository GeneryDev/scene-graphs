	...
	for child: Node in editor.get_children():
		...
		for connection in editor.get_connection_list_from_node(graph_node.name):
			if connection.to_node != graph_node.name:
				continue
			if !editor.check_connection_port_types(connection, editor.port_type(&"node_reference_out"), editor.port_type(&"node_reference")):
				continue
...
func _on_disconnection_request(from_node_name: StringName, from_port: int, to_node_name: StringName, to_port: int) -> void:
	...
	if (from_graph_node as GraphNode).get_output_port_type(from_port) != editor.port_type(&"node_reference_out"):
		return
	if (to_graph_node as GraphNode).get_input_port_type(to_port) != editor.port_type(&"node_reference"):
		return
